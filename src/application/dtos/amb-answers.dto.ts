...
export class AmbAnswersDto {
  static fromRequest(req: Request): [string?, RequestAnswerInterface?] {
    ...
    if (!checklistAmbulanceId)
      return [ERROR_CODES.MISSING_CHECKLIST_AMBULANCE_ID];
    ...
    for (let i = 0; i < answers.length; i++) {
      ...
      out.push({
        ...
        ...(a.valueOption !== undefined
          ? { valueOption: String(a.valueOption) }
          : {}),
        ...(a.valueText !== undefined
          ? { valueText: String(a.valueText) }
          : {}),
      });
    }
