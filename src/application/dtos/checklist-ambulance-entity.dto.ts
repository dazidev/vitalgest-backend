...
export class CheckListAmbulanceEntityDto {
  ...
  private static validateData (ambulanceId: string, shiftId: string, km: string, gasFile: File, signOperatorFile: File, signRecipientFile: File) {
  ...
  static create(object: {[key: string]: any}): [string?, CheckListAmbulanceEntityDto?] {
    const { ambulanceId, shiftId, km, gasFile, signOperatorFile, signRecipientFile, notes } = object
    
    const error = this.validateData(ambulanceId, shiftId, km, gasFile, signOperatorFile, signRecipientFile)
    if (!(error === true)) return [error]

    return [undefined, new CheckListAmbulanceEntityDto({ambulanceId, shiftId, km, gasFile, signOperatorFile, signRecipientFile, notes})]
  }
