...
export class CheckListSupplyEntityDto {
  ...
  static create(object: { [key: string]: any }): [string?, CheckListSupplyEntityDto?] {
  ...
  static sign(object: { [key: string]: any }): [string?, CheckListSupplyEntityDto?] {
    const { id, signOperatorFile, signRecipientFile } = object

    if (!id) return [ERROR_CODES.MISSING_CHECKLIST_SUPPLY_ID]
    if (!regularExp.uuid.test(id)) return [ERROR_CODES.INVALID_CHECKLIST_SUPPLY_ID]

    const error = this.validateSign(signOperatorFile, signRecipientFile)
    if (!(error === true)) return [error]

    return [undefined, new CheckListSupplyEntityDto({ id, signOperatorFile, signRecipientFile })]
  }

  static delete(object: { [key: string]: any }): [string?, CheckListSupplyEntityDto?] {
    const { id } = object
