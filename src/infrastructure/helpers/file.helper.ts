...
export async function saveWebFile(
  ...
  if (!WebFile) {
    throw new Error(
      "File API no disponible. Usa Node >= 18 o undici con File."
    );
  }
  ...
  //* crea subcarpeta
  const now = new Date();
  const defaultSub = `${now.getFullYear()}-${String(
    now.getMonth() + 1
  ).padStart(2, "0")}`;
  ...
  const hasExt =
    path.extname(safeOriginal).toLowerCase() === `.${ext.toLowerCase()}`;
  ...
  try {
    ...
    const retryName = `${randomUUID()}-${safeOriginal}${
      hasExt ? "" : `.${ext}`
    }`;
