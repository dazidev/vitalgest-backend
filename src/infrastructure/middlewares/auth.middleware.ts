...
export class AuthMiddleware {
  static async validateJWT(req: Request, _res: Response, next: NextFunction) {
    ...
    if (!authorization)
      return next(CustomError.unauthorized(ERROR_CODES.NO_TOKEN_PROVIDED));
    if (!authorization.startsWith("Bearer "))
      return next(CustomError.unauthorized(ERROR_CODES.INVALID_BEARER_TOKEN));

    const token = authorization.split(" ").at(1) || "";
    try {
      const result = await JwtAdapter.validateAccessToken<{ id: string }>(
        token
      );
      if (!result.success) {
        if (result.reason === "expired")
          return next(CustomError.unauthorized(ERROR_CODES.TOKEN_EXPIRED));
      ...
      if (!user)
        return next(CustomError.badRequest(ERROR_CODES.INVALID_TOKEN_USER));
      if (user.status === false)
        return next(CustomError.badRequest(ERROR_CODES.USER_NOT_ACTIVE));
