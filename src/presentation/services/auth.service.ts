...
export class AuthService implements AuthServiceInterface {
  ...
  async loginUser(userEntityDto: UserEntityDto): Promise<LoginServiceResponse> {
    ...
    const { delegation_id: delegationId, ...userEntity } =
      UserEntity.login(user);
    ...
    const tokenRefresh = await JwtAdapter.generateToken(
      payload,
      "7d",
      "REFRESH"
    );
  ...
  async newAccessToken(refreshTokenReq: string): Promise<TokenServiceResponse> {
    const result = await JwtAdapter.validateRefreshToken<{ id: string }>(
      refreshTokenReq
    );
    if (!result.success) {
